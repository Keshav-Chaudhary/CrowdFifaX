...
const critical = sorted.filter(r => r.risk === 'CRITICAL');
console.log(`CRITICAL: ${critical.length}, HIGH: ${sorted.filter(r => r.risk === 'HIGH').length}, MEDIUM: ${sorted.filter(r => r.risk === 'MEDIUM').length}`);
if (critical.length > 0) {
    console.log('\nCRITICAL files (production source only):');
    critical.filter(r => !r.file.includes('.test.')).forEach(r => {
        console.log(`  ${r.file}: largestFuncLines=${r.largestFuncLines}, maxDepth=${r.maxDepth}, loc=${r.loc}`);
    });
}
